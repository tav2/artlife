                ...
                <div class="uk-form-row">
                    <select name="tovar_select" id="" onchange="show_tovar(this);" class="uk-width-4-10">
                        ...
                        <?php foreach($tovar_menu as $id): ?>
                            <option value="<?php echo $id['tovar_id'] ?>"><?php echo $id['tovar_name'] ?></option>
                        <?php endforeach ?>
                    </select>
                    <input type="submit" value="Применить" class="uk-button uk-width-4-10"/>
                </div>
        ...
        <div class="uk-width-1-1">
                ...
                <div class="uk-form-row">
                        <?php foreach($tovar as $id): ?>
                            <?php echo $id['tovar_name'] ?><br>
                            <?php echo "Цена:".$id['tovar_prise'] ?>
                            <?php echo "Цена АБ:".$id['tovar_prise_ab'] ?>
                            <?php echo "Цена В:".$id['tovar_prise_b'] ?>
                            <?php echo "Цена С:".$id['tovar_prise_c'] ?><br>
                            <?php echo "Скидка:".$id['tovar_skidka'] ?><br>
                            <?php echo "Текст:".$this->dbmodel->cutStr($id['tovar_text']) ?><br>
                            <?php echo "Состав:".$this->dbmodel->cutStr($id['tovar_sostav']) ?><br>
                            <?php echo "Способ применения:".$this->dbmodel->cutStr($id['tovar_sposob']) ?><br>
                            <?php echo "Противопоказания:".$this->dbmodel->cutStr($id['tovar_protivopokaz']) ?><br>
                            <?php echo "Рейтинг:".$id['tovar_reit'] ?><br>
                            <?php echo "Размеры:".$id['tovar_razmer'] ?><br>
                            <?php echo "Другое:".$id['tovar_uther'] ?><br>
                            <?php echo "Доступность:".$id['tovar_dostupnost'] ?><br>
