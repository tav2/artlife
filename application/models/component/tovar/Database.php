...
class Database extends MY_Model {
    ...
    public function get_category_name($tovar_category_id)
    {
    ...
    public function get_sub_category_catname($tovar_category_id)
    {
        $this->db->where('tovar_category_id', $tovar_category_id);
        $rez = $this->db->get('tovar_category');
        foreach ($rez->result_array() as $item) {$rez2 = $item['tovar_category_name'] ;}
    ...
    public function get_sub_category_name($tovar_sub_category_id)
    {
        $this->db->where('tovar_sub_category_id', $tovar_sub_category_id);
    ...
    function get_tovar_category($tovar_category_id)
    {
        $this->db->where('tovar_category_id', $tovar_category_id);
    ...
    //удаление подкатегории вместе со всеми товарами
    public function delete_sub_category($id)
    {
        ...
        {
            $this->dell_all_img($item['tovar_id']);
            $this->dell_tovar($item['tovar_id']);
        }

        $this->delete_sub_category_img($id);

        $this->db->where('tovar_sub_category_id', $id);
        $this->db->delete('tovar_sub_category');
    }

    // удаление категории вместе со всеми подкатегориями и товарами
    public function delete_category($id)
    {
        ...
        foreach ($rez->result_array() as $item) 
        {
            $this->delete_sub_category($item['tovar_sub_category_id']);
        }

        $this->delete_category_img($id);
        $this->db->where('tovar_category_id', $id);
    ...
    public function delete_sub_category_img($id)
    {
        ...
        $rez = $this->db->get('tovar_sub_category');
        foreach ($rez->result_array() as $item) {$put = $item['tovar_img_adres'] ; $name = $item['tovar_sub_category_name'] ;}
        if ($put) {
            unlink($put);
        }
        ...
        $data = array(
               'tovar_sub_category_name' => $name,
               'tovar_category_id' => $id,
               'tovar_img_adres' => '',
            );
    ...
    public function delete_category_img($id)
    {
        ...
        $rez = $this->db->get('tovar_category');
        foreach ($rez->result_array() as $item) {$put = $item['tovar_img_adres'] ; $name = $item['tovar_category_name'] ;}
        unlink($put);

        $data = array(
               'tovar_category_name' => $name,
               'tovar_img_adres' => '',
            );

        $this->db->where('tovar_category_id', $id);
        $this->db->update('tovar_category', $data); 
    }
    ...
    public function add_data_category($data, $id)
    {
        If ($id!='')
        {
            $this->db->where('tovar_category_id', $id);
        ...
        else $this->db->insert('tovar_category', $data); 
    }
