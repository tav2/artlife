...
class Tovar extends Admin_Controller {
	...
	public function action_delete_category_img($id)
	{
		$this->dbmodel->delete_category_img($id);
	...
	{
		$data['tovar_category'] = array();

		// если нажали выбрать категорию
		if($this->input->post('tovar_category_select'))
		{
			$data['tovar_category'] = $this->dbmodel->get_tovar_category($this->input->post('tovar_category_select'));
		}
		...
		$this->render('dashboard', array(
			...
			'component' => $this->load->view('component/tovar/edit_category', array(
				...
				'tovar_category' => $data['tovar_category']
			), true)
		));
	}
	...
	public function action_edit_sub_category()
	{
		...
		if($this->input->post('tovar_sub_category_select'))
		{
			...
			$data['tovar_sub_category_img'] = $this->dbmodel->get_tovar_sub_category($this->input->post('tovar_sub_category_select'));
	...
	public function action_add_category($id='')
	{
		...
		if($this->input->post('category'))
		{
			...
			$this->dbmodel->add_data_category($data, $id);
		...
		{
			foreach ($this->dbmodel->get_tovar_category($id) as $item) 
			{
				$category_red['tovar_category_name'] = $item['tovar_category_name'];
			}
		}
		else
		{
			$category_red['tovar_category_name'] = '';
		}
	...
	public function action_add_tovar($id='')
	{
		...
		if($this->input->post('tovar_name'))
		{
			...
			$data = array(
				...
				'tovar_sostav' => $this->input->post('tovar_sostav'),
				'tovar_sposob' => $this->input->post('tovar_sposob'),
				'tovar_protivopokaz' => $this->input->post('tovar_protivopokaz'),
				...
				'tovar_new' => $this->input->post('tovar_new'),
		...
		if($id)//если редактируем
		{
			...
			foreach ($this->dbmodel->get_tovar($id) as $item) 
			{
				...
				$tovar_red['tovar_sostav'] = $item['tovar_sostav'];
				$tovar_red['tovar_sposob'] = $item['tovar_sposob'];
				$tovar_red['tovar_protivopokaz'] = $item['tovar_protivopokaz'];
		...
		else 
		{
				...
				$tovar_red['tovar_sostav'] = '';
				$tovar_red['tovar_sposob'] = '';
				$tovar_red['tovar_protivopokaz'] = '';
	...
	public function action_add_sub_category_img($id)
	{
			...
			else 
			{
				...
				$add['tovar_img_adres'] = $config['upload_path'].$upload_data['file_name']; // сохраняем имя файла в элемент массива add
				// $add['tovar_img_type'] = $this->input->post('tovar_img_type'); // сохраняем тип изображения в элемент массива add
				$add['tovar_sub_category_name'] = $this->dbmodel->get_sub_category_name($id); // 
				$this->db->where('tovar_sub_category_id', $id);
				$this->db->update('tovar_sub_category', $add); // заносим это значение в таблицу 
				/* Конец занесения имени файла в БД*/
	...
	public function action_add_category_img($id)
	{
			// создаем папку если ее нет
			$pass = 'assets/img/category/';
			...
			else 
			{
				...
				$add['tovar_img_adres'] = $config['upload_path'].$upload_data['file_name']; // сохраняем имя файла в элемент массива add
				// $add['tovar_img_type'] = $this->input->post('tovar_img_type'); // сохраняем тип изображения в элемент массива add
				$add['tovar_category_name'] = $this->dbmodel->get_category_name($id); // 
				$this->db->where('tovar_category_id', $id);
				$this->db->update('tovar_category', $add); // заносим это значение в таблицу 
				/* Конец занесения имени файла в БД*/
			...
			$this->render('dashboard', array(
				...
				'js' => $this->asset->js(array('jquery', 'uikit', 'moment')),
				'component_menu' => $this->component->get_navigation(),
				'component' => $this->load->view('component/tovar/add_category_img', array(
					// 'all_category_select' => $this->dbmodel->get_all_tovar();
					'id' => $id
				), true)
			));
